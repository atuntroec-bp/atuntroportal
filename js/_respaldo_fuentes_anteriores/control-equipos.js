...
function renderWinchesTab(){
  ...
  <tr>${v.viajes.map(()=>`<th class="sub">Horas</th><th class="sub">Lances</th>`).join('')}</tr>
  </thead><tbody>`;
  v.winches.forEach(w=>{
    const est = calcEstadoWinche(w);
    html += `<tr>
...
function updateCableField(id,field,val){ const c=findCable(id); if(!c) return; c[field]=val; persist(); refreshCableRow(id); }
