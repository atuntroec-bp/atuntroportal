...
const GT = (() => {
  ...
  /* ── Stats ──────────────────────────────────────────── */
  function stats() {
    const c = tasks.filter(t => t.estado === 'COMPLETO').length;
    const i = tasks.filter(t => t.estado === 'INCOMPLETO').length;
    const p = tasks.filter(t => t.estado === 'PENDIENTE').length;
  ...
  function renderDonut() {
    ...
    const slices = [
      ...
      { label:'Incompleto', val:s.i, key:'INCOMPLETO' },
  ...
  function renderBars() {
    ...
    `<div style="display:flex;gap:14px;margin-top:10px;padding-top:10px;border-top:1px solid rgba(0,0,0,.06)">
      ${['Completo','Incompleto','Pendiente'].map((l,i) => {
  ...
  function statusBadge(e) {
    return `<span class="gt-badge gt-st-${e}">${esc(e)}</span>`;
  }
  ...
  function buildRespFilter() {
    ...
    const cur = sel.value;
    const active = [...new Set(tasks.map(t => t.resp))].sort();
  ...
  function applyFilters() {
    ...
    if (filtered) info.textContent = `Mostrando ${visible.length} de ${tasks.length} tareas`;
  ...
  function openModal(id) {
    ...
    if (id) {
      ...
      document.getElementById('gm-dept').value   = 'Administrativo';
  ...
  /* ── Init ───────────────────────────────────────────── */
  function init() { load(); renderAll(); startRealtimeSync(); }
