  ...
  function hideAllScreens() {
    ['screen-login','screen-dashboard','screen-gestor','screen-checklist-ops','screen-checklist-adm','screen-equipos','screen-simulador'].forEach(function(id){
  ...
  var _pendingCollapsed = false, _pendingCollapsedGt = false;
  function loadPendingTasks() {
    try {
      var raw = localStorage.getItem('atuntro_gestor_v1');
      var data = raw ? JSON.parse(raw) : null;
      var tasks = data ? (data.tasks || []) : [];
      var pending = tasks.filter(function(t){ return t.estado !== 'COMPLETO'; });
      renderPendingWidget('pending-tasks-list', 'pending-count-badge', pending);
      renderPendingWidget('pending-tasks-list-gt', 'pending-count-badge-gt', pending);
    } catch(e) {
      ['pending-tasks-list','pending-tasks-list-gt'].forEach(function(id){
        var el = document.getElementById(id);
        if (el) el.innerHTML = '<div class="pending-empty"><div class="pending-empty-icon">⚠️</div>No se pudo leer el Gestor de Tareas.</div>';
      });
    }
  }
  ...
  function togglePendingTasksGestor() {
    _pendingCollapsedGt = !_pendingCollapsedGt;
    var b=document.getElementById('pending-tasks-body-gt'), ic=document.getElementById('pending-toggle-icon-gt');
    if(b) b.className='pending-tasks-body '+(_pendingCollapsedGt?'collapsed':'expanded');
    if(ic) ic.textContent=_pendingCollapsedGt?'▼':'▲';
  }
